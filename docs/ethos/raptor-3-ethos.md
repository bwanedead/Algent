...
This ethos applies to code, prompts, doctrine, docs, UI flows, contracts, and process design.
...
- generic guidance in generic surfaces
...
## Raptor 3 For System Boundaries

The ethos reinforces a clean split between mechanics and meaning:

- the mechanical layer owns rails, validation, persistence, execution, and observability
- the domain layer owns semantics, truth, and closure
- deterministic code must not silently author the meaning it is only supposed to carry

A native system has one trunk for shared mechanics. It does not preserve domain-private adapters as generic law, and it does not keep old execution metaphors alive inside the current runtime.

If a refactor leaves the old engine running inside the new shell, call it mixed or patchwork. If it makes the native path direct and makes old pieces deletable, it is converging.
