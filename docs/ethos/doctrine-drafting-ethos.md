...
> Gather the full work scope before acting.

Better:

> Map the work universe early because later turns inherit its shape. Items discovered late cannot be batched with nearby work, cause backtracking, increase prompt cost, and make the run feel less trustworthy to the user.
...
- If the evidence does not contain the target, it is not evidence for that target.
...
Domain doctrine may use domain vocabulary, but it should still avoid overfitting to one artifact, one run failure, or one observed example. Use incidents to discover general failure modes. Do not paste the incident into doctrine unless the incident itself is a stable domain pattern.

Bad:

> List the three specific conflicts we hit last run before moving.

Better:

> List all decision-critical values, references, contradictions, source limits, dependencies, and downstream-governing choices before resolution motion.
...
> Note: the examples below are placeholders. Substitute this project's real action/state
> vocabulary as it is defined (e.g. run events, artifact writes, tool calls, run context).

Illustrative examples:

- a batch verb when teaching multiple related actions in one turn
- a next-turn setup verb when teaching look-ahead
- a durable-attention verb when teaching persistent focus
- a delegation verb when teaching isolated observations
- a state-write verb when teaching durable integration
...
Good doctrine integrates related ideas that are part of one motion pattern, and explains them together. Bad doctrine stacks those as unrelated reminders.
...
- mission method
- work universe and scope
- units of work and their grouping
- evidence standard
