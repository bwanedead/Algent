# PROMPT.md — Ralph Run: 2026-01-29__cockpit

You are an autonomous coding agent working in this repository under a Ralph loop.

IMPORTANT: This exact prompt will be re-run repeatedly each iteration. You must use repo state (files + run logs) as memory.
...
4. Read `ralph/runs/2026-01-29__cockpit/PRD.md`
5. Read `ralph/runs/2026-01-29__cockpit/prd.json`
...
- Respect repo rules: do NOT run git commands or install dependencies unless explicitly instructed by the maintainer.
...
- Preferred: count files in `ralph/runs/2026-01-29__cockpit/transcripts/` and add 1.
- Fallback: count existing `- Iteration:` entries in `ralph/runs/2026-01-29__cockpit/progress.md` and add 1.
...
2. Ensure `ralph/runs/2026-01-29__cockpit/transcripts/` exists.
...
7. If repo rules allow, commit with: `Ralph 2026-01-29__cockpit: <story id> <story title>`. If not allowed, skip and note in `progress.md`.
8. Update `ralph/runs/2026-01-29__cockpit/prd.json`:
   - set that story `"passes": true` ONLY if acceptance criteria are satisfied
9. Append to `ralph/runs/2026-01-29__cockpit/progress.md` (format below)
10. Append to `ralph/runs/2026-01-29__cockpit/SUMMARY.md` (format below)
11. Write a transcript for this iteration:
    - Use `ralph/templates/TRANSCRIPT_TEMPLATE.md`
    - Save to `ralph/runs/2026-01-29__cockpit/transcripts/iter-XXXX.md` (zero-padded, 4 digits)
...
- Perform a short review pass using `ralph/templates/REVIEW_TEMPLATE.md` and write to `ralph/runs/2026-01-29__cockpit/review.md` (overwrite is fine).
- Only pause story shipping if the review indicates drift or risk; otherwise continue with the normal story workflow.
- If review says "off course," write steering guidance using `ralph/templates/STEERING_TEMPLATE.md` to `ralph/runs/2026-01-29__cockpit/steering.md` (overwrite is fine) and keep scope tiny.
...
If `ralph/runs/2026-01-29__cockpit/loop_state.json` exists, you must obey edit-permission rules in `ralph/templates/CONTROL_PLANE.md`.
