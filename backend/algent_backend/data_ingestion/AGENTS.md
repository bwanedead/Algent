...
- **Synthesis defaults ON** via `agent_system/agents/newsroom/flags.py` (`SYNTHESIS_ENABLED`, target `SYNTHESIS_TARGET_VECTORS`). Agents flip that file — do not ask the human to set `.env`. Optional one-shot: `ALGENT_SYNTHESIS=0/1`.
