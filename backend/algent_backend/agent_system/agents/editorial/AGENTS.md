...
- The analytics worker (figures) starts on a background thread as soon as the analytics plan exists and is joined after the prose/headline/hero finish. It reads only plan + pre-draft profile; do not make it depend on the draft.
- Order after the first draft: cut → comprehension → caveat check (+ surgical hedge repair). The caveat check must stay LAST — it is the only lane with the evidence grades, and its verdict must describe the prose that ships.
